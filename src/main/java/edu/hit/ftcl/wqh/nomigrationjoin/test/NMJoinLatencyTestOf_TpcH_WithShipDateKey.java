...
import edu.hit.ftcl.wqh.common.fortest.CommonKafkaInputStreamGenerate;
import edu.hit.ftcl.wqh.common.fortest.CommonTestDataGenerateParameters;
import edu.hit.ftcl.wqh.common.fortest.ComputeResultTupleLatencyMapFunction;
import edu.hit.ftcl.wqh.common.fortest.ParallelAccumulateResultLatencyMapFunction;
import edu.hit.ftcl.wqh.common.fortest.forTpcH.TpcFilterFor_L1;
import edu.hit.ftcl.wqh.common.fortest.forTpcH.TpcFilterFor_L2;
import edu.hit.ftcl.wqh.common.fortest.forTpcH.TpcTuple4Filter;
import edu.hit.ftcl.wqh.common.fortest.forTpcH.TranslateTpcStringToTuple4WithShipDateKeyMap;
...
public class NMJoinLatencyTestOf_TpcH_WithShipDateKey {
...
//        DataStream<Tuple4<String, String, String, String>> secondStream = env.addSource(new TestCorrectResourceDemo()).assignTimestampsAndWatermarks(new MyCommonTestWatermarkPeriodicAssigner());
        ...
        DataStream<Tuple4<String, String, String, String>> firstStream = CommonKafkaInputStreamGenerate.getFirstInputStream(env)
                .map(new TranslateTpcStringToTuple4WithShipDateKeyMap("first")).setParallelism(CommonTestDataGenerateParameters.KAFKA_R_TOPIC_PARTITIONS_NUM)
                .filter(new TpcFilterFor_L1()).setParallelism(CommonTestDataGenerateParameters.KAFKA_R_TOPIC_PARTITIONS_NUM)
        ...
        DataStream<Tuple4<String, String, String, String>> secondStream = CommonKafkaInputStreamGenerate.getSecondInputStream(env)
                .map(new TranslateTpcStringToTuple4WithShipDateKeyMap("second")).setParallelism(CommonTestDataGenerateParameters.KAFKA_S_TOPIC_PARTITIONS_NUM)
                .filter(new TpcFilterFor_L2()).setParallelism(CommonTestDataGenerateParameters.KAFKA_S_TOPIC_PARTITIONS_NUM)
                .assignTimestampsAndWatermarks(new MyCommonTestWatermarkPeriodicAssigner());
